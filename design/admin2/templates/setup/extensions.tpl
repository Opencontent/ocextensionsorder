{def $extensions_order = extensions_order( $available_extension_array )}
...
<tr>
    ...
    <th><span style="cursor:pointer">{'Name'|i18n( 'design/admin/setup/extensions' )}</span></th>
    <th><span style="cursor:pointer">{'Loading order'|i18n( 'extension/ocextensionsorder' )}</span></th>
</tr>
</thead>
<tbody>
...
<tr class="{$Extensions.sequence}">
    ...
    <td>
        {foreach $extensions_order as $key => $value}
            {if $value|eq( $Extensions.item )}
                {$key}
            {/if}   
        {/foreach}
    </td>
